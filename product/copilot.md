...
接下来，就让我们体验一下Copilot的神奇之处，验证一下它生成的代码质量如何。这里要注意一点，Copilot是使用编程语言中的注释最为交互方式的，比如对于Javascript来说，输入`//`就可以激活Copilot，然后它会根据我们代码的上下文以及我们已经输入的部分注释内容，在代码库中匹配出合适的内容推荐出来，如果我们认为推荐的合适，可以按 `tab` 键应用推荐内容。然后按下回车开始进行下一行内容的提示。
